...
domain_generic = [
    ...
    'музея', 'музей', 'музее', 'третьяковк', 'третьяковская', 'третьяковской', 'третьяковку', 'галерея', 'галереи', 'галерее', 'третьякова',
    'имени', 'государственный', 'государственного', 'государственных',
    'изобразительных', 'подробнее', 'представляет', 'выставку', 'лаврушинском', 'лаврушинский',
...
def get_all_keyword_scores(text_series):
    ...
    {слово: средний вес НА ДОКУМЕНТ}.

    Важно: используем среднее (sum / n_documents), а не сырую сумму.
    Сумма растёт вместе с размером корпуса просто потому, что в нём больше
    документов — а до/после 2022 корпуса разного размера, поэтому сырые
    суммы искусственно завышают "до 2022". Среднее на документ убирает
    этот эффект и делает периоды сравнимыми.
    """
    vec = TfidfVectorizer(
        stop_words=all_stop,
        max_features=1500,
...
keywords_df.to_csv('tretyakov_tfidf_keywords.csv', index=False)
print("\nСохранено: tretyakov_tfidf_keywords.csv")
...
ax.set_title(f'{MUSEUM_LABEL}: Top Keywords (Mean TF-IDF per Document), Before vs. After 2022', fontsize=15, pad=16)
ax.legend(fontsize=11)

plt.tight_layout()
plt.savefig('tretyakov_tfidf_keywords.png', dpi=300, bbox_inches='tight')
print("\nСохранено: tretyakov_tfidf_keywords.png")