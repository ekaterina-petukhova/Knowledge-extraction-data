...
ARCHIVE_URL = "https://www.hermitagemuseum.org/what-s-on/exhibitions-archive/?lng=ru&page=0"
...
def collect_links_on_current_page(page):
    ...
    for link in links:
        ...
        # Реальные страницы выставок у Эрмитажа обычно:
        # https://www.hermitagemuseum.org/what-s-on/slug
        if not href.startswith("https://www.hermitagemuseum.org/what-s-on/"):
            continue

        slug = href.replace("https://www.hermitagemuseum.org/what-s-on/", "").strip("/")
...
def click_pagination_button(page, page_number):
    """
    Для Эрмитажа не нажимаем кнопку, а переходим на URL с page=N.
    Оставляем функцию с тем же названием, чтобы структура была как в коде Третьяковки.
    """
    url = f"https://www.hermitagemuseum.org/what-s-on/exhibitions-archive/?lng=ru&page={page_number - 1}"
    ...
    try:
        ...
        print(f"Не удалось перейти на страницу {page_number}:", e)
        return False
...
def collect_all_archive_links():
    ...
    with sync_playwright() as p:
        ...
        # По твоему логу архив Эрмитажа примерно до 62 страницы.
        for page_number in range(2, 63):
...
def main():
    ...
    df_links.to_csv("hermitage_archive_links_all_pages.csv", index=False, encoding="utf-8-sig")
    print("Ссылки сохранены: hermitage_archive_links_all_pages.csv")
    ...
    df.to_csv("hermitage_exhibitions_2014_2026.csv", index=False, encoding="utf-8-sig")

    print("\nГотово!")
    print("Собрано выставок 2014–2026:", len(df))
    print("Файл сохранен: hermitage_exhibitions_2014_2026.csv")
