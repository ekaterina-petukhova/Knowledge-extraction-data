...
INPUT_FILE = 'art_newspaper_classified.csv'
...
def get_all_keyword_scores(text_series):
    ...
    документов — а до/после 2022 у нас корпуса разного размера (291 vs 147),
    поэтому сырые суммы искусственно завышают "до 2022". Среднее на документ
    убирает этот эффект и делает периоды сравнимыми.
    """
    vec = TfidfVectorizer(
        stop_words=all_stop,
        max_features=1500,
